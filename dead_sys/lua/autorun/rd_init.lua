...
local MaxDRagsVar = CreateConVar("ragdeath_keepmax",DS_MAX_RAGDOLL,FCVAR_ARCHIVE,"")
...
local TimeRagVar = CreateConVar("ragdeath_timeremove",DS_MAX_TIME_RAGDOLL, FCVAR_ARCHIVE,"")
...
local function createRagdoll(player)
	...
	Ragdoll:SetNWBool( 'dead_player_ragdoll' , true)
	Ragdoll:SetNWString('dead_player_ragdoll_why', player:GetAttacker())
	...
	if timedgo>0 then
	  timer.Simple(timedgo,function() if IsValid(Ragdoll) then 
	  	Ragdoll:Remove()
